...
// given a vector t initializes a cube with t as the center
struct cube MakeCube(struct vector t, struct rgb color) {
	struct cube cube;
	int i;

	for (i=0 ; i < 8 ; i++)
		*(cube.v+i) = VectorAdd(*(STD_CUBE.v+i), t); 
...
void RenderObject(struct vector * vertices, int nv, struct triangle * triangles, int nt) {
...
//stuff
	struct cube cube;
	struct vector t = {0, 0, 7};
	float r = sqrt(8)/2;
	float i;


//	for(i = 0 ; i < 8 ; i++) {
//	printf("%f,%f,%f\n", cube.v[i].x, cube.v[i].y, cube.v[i].z);	
//	};
...
//	printf("\n");
	...
	i = 0;
	while(1) {
	cube = MakeCube(t, green);

	cube.v[0].x = r * cos(i - 0.785) + t.x;
	cube.v[0].z = r * sin(i - 0.785) + t.z;
	
	cube.v[1].x = r * cos(i - 2.356) + t.x;
	cube.v[1].z = r * sin(i - 2.356) + t.z;

	cube.v[4].x = r * cos(i + 0.785) + t.x;
	cube.v[4].z = r * sin(i + 0.785) + t.z;

	cube.v[5].x = r * cos(i + 2.356) + t.x;
	cube.v[5].z = r * sin(i + 2.356) + t.z;

	cube.v[3].x = r * cos(i - 0.785) + t.x;
	cube.v[3].z = r * sin(i - 0.785) + t.z;

	cube.v[2].x = r * cos(i - 2.356) + t.x;
	cube.v[2].z = r * sin(i - 2.356) + t.z;

	cube.v[7].x = r * cos(i + 0.785) + t.x;
	cube.v[7].z = r * sin(i + 0.785) + t.z;

	cube.v[6].x = r * cos(i + 2.356) + t.x;
	cube.v[6].z = r * sin(i + 2.356) + t.z;

	RenderObject(cube.v, CUBE_VERTS, cube.t, CUBE_TRIANGLES);
...
//	struct point p0 = {-200, -250, 0}, p1 = {200, 50, 0}, p2 = {20, 250, 1};
	
//	DrawShadedTriangle(p0, p1, p2, pink);
