...
struct vector Transform(struct vector v, float * m) {
	struct vector result;
...
struct vector MatrixMul(float * m, struct vector v, int n) {
...
//stuff
	...
	struct scene scene1 = {
		{
			cube, 1, 0, 0, 0, {-1.5, 0, 7}, Nepeta, //model, scale, rotation, axes, vector
			cube, 0.5, 0, 0, 0, {1.5, 1.5, 7}, Pawl,
			pyramid, 1, 0, 0, 0, {1.5, -0.5, 7}, Aradia,
		},
		3
	};	
	...
	while (1) {
		...
		scene1.instances[0].transform.x_rotation = i;
