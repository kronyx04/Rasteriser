...
struct vector VectorRot(struct vector v, float radians) {
	struct vector result;
	float radius = sqrt(fabs(v.x) * fabs(v.x) + fabs(v.z) * fabs(v.z));
	float base_angle = atan2(fabs(v.z),fabs(v.x));
//	printf("%f", base_angle);
	float true_angle;
...
struct vector ApplyTransform(struct vector vertex, struct transform transform) {
	...
	rotated = VectorRot(scaled, transform.rotation);
...
//stuff
	...
	struct scene scene1 = {
		{
			cube, 1, 0, {-1.5, 0, 7}, Nepeta,
			cube, 0.5, 0, {1.5, 1, 7}, Pawl,
			pyramid, 1.25, 0, {1.5, -1, 7}, Aradia,
		},
		3
	};	
	...
	while (1) {
		...
		scene1.instances[0].transform.rotation = i * 3;
